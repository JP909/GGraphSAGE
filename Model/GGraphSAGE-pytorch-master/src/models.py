...
class Classification(nn.Module):
	...
	def __init__(self, emb_size, num_classes):
		...
		self.layer = nn.Sequential(
								nn.Linear(emb_size, num_classes)	#线性层将128维转换成2维特征
								#nn.ReLU()
							)
		self.init_params()
...
##无监督学习增加负采样
	def extend_nodes(self, nodes, num_neg=20):
		self.positive_pairs = []
		self.node_positive_pairs = {}
		self.negtive_pairs = []
		self.node_negtive_pairs = {}
...
class SageLayer(nn.Module):
	...
	def __init__(self, input_size, out_size, gcn=False): 
		...
		self.input_size = input_size #9维，第二层128维
		self.out_size = out_size #128维


		self.gcn = gcn
		##weight是128*18维因为聚合邻居信息，所以*2
		self.weight = nn.Parameter(torch.FloatTensor(out_size, self.input_size if self.gcn else 2 * self.input_size))##concat邻居节点，所以设置为2倍，不用gcn的话不管
...
class GraphSage(nn.Module):
	...
	def __init__(self, num_layers, input_size, out_size, raw_features, adj_lists, device, gcn=False, agg_func='MEAN'):
		...
		self.input_size = input_size ##输入特征的维度9
		self.out_size = out_size ##128维
		self.num_layers = num_layers ##2层
		self.gcn = gcn #是否用GCN，false
		self.device = device
		self.agg_func = agg_func #聚合函数mean

		self.raw_features = raw_features #特征矩阵
		self.adj_lists = adj_lists #边的链接字典

		for index in range(1, num_layers + 1):
			layer_size = out_size if index != 1 else input_size  ##如果是第一层，维度=inputsize:9,如果是第二层,维度=output维度
	...
	def forward(self, nodes_batch):
		...
		for i in range(self.num_layers):
			...
			nodes_batch_layers.insert(0, (lower_layer_nodes, lower_samp_neighs, lower_layer_nodes_dict)) ##节点插入

		assert len(nodes_batch_layers) == self.num_layers + 1

		pre_hidden_embs = self.raw_features
		for index in range(1, self.num_layers+1):
			nb = nodes_batch_layers[index][0] ##聚合节点和周围节点
			pre_neighs = nodes_batch_layers[index-1]##取最外层所有邻居节点的邻居请况
			...
			logists = torch.log_softmax(pre_hidden_embs, 1)
	...
	def _nodes_map(self, nodes, hidden_embs, neighs):
		...
		return index
##得到这个batch节点的邻居节点,取它至多10个邻居
	def _get_unique_neighs_list(self, nodes, num_sample=10):
		_set = set
		to_neighs = [self.adj_lists[int(node)] for node in nodes]#self.adj_lists边矩阵,获取节点邻居
		if not num_sample is None:##对邻居节点采样，如果大于邻居数据，则采样,对某些邻居采样，而不是所有邻居
		...
		return samp_neighs, unique_nodes, _unique_nodes_list ##samp_neighs所有邻居节点的集合,所有节点的字典,所有的节点
#聚合所有节点
	def aggregate(self, nodes, pre_hidden_embs, pre_neighs, num_sample=10):
		...
		if self.agg_func == 'MEAN':
			num_neigh = mask.sum(1, keepdim=True) #按行求和
			mask = mask.div(num_neigh).to(embed_matrix.device)
			aggregate_feats = mask.mm(embed_matrix) ##将邻居加在一起
